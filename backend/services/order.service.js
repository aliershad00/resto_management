...
module.exports = {
  ...
  async createOrder(order, user) {
    try {
      const { deliveryAddress, restaurantId } = order;

      if (!deliveryAddress || !restaurantId) {
        throw new Error("Delivery address and restaurant ID are required.");
      }


      // Save or find the delivery address
      let savedAddress;
      if (deliveryAddress._id) {
        const isAddressExist = await Address.findById(deliveryAddress._id);
        savedAddress = isAddressExist
          ? isAddressExist
          : await new Address(deliveryAddress).save();
      } else {
        savedAddress = await new Address(deliveryAddress).save();
      }
