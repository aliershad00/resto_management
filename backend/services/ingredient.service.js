...
module.exports = {
  ...
  async createIngredientsItem(
    ...
    try {
      ...
      // Create a new ingredient item (without using .populate())
      ...
      // Populate the saved item with category data
      const populatedItem = await IngredientsItem.findById(savedItem._id).populate(
        "category"
      );

      // Add the new item's ID to the category's ingredients array
